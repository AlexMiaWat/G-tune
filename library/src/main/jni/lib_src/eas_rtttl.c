...
/* local prototypes */
static EAS_RESULT
RTTTL_CheckFileType(S_EAS_DATA *pEASData, EAS_FILE_HANDLE fileHandle, EAS_VOID_PTR *ppHandle,
                    EAS_I32 offset);
...
static EAS_RESULT
RTTTL_SetData(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_I32 param, EAS_I32 value);

static EAS_RESULT
RTTTL_GetData(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_I32 param, EAS_I32 *pValue);

static EAS_RESULT RTTTL_GetStyle(EAS_HW_DATA_HANDLE hwInstData, S_RTTTL_DATA *pData);

static EAS_RESULT
RTTTL_GetDuration(EAS_HW_DATA_HANDLE hwInstData, S_RTTTL_DATA *pData, EAS_I8 *pDuration);

static EAS_RESULT
RTTTL_GetOctave(EAS_HW_DATA_HANDLE hwInstData, S_RTTTL_DATA *pData, EAS_U8 *pOctave);

static EAS_RESULT RTTTL_GetTempo(EAS_HW_DATA_HANDLE hwInstData, S_RTTTL_DATA *pData);

static EAS_RESULT
RTTTL_GetNumber(EAS_HW_DATA_HANDLE hwInstData, S_RTTTL_DATA *pData, EAS_I32 *pValue);

static EAS_RESULT RTTTL_ParseHeader(S_EAS_DATA *pEASData, S_RTTTL_DATA *pData, EAS_BOOL metaData);

static EAS_RESULT
RTTTL_GetNextChar(EAS_HW_DATA_HANDLE hwInstData, S_RTTTL_DATA *pData, EAS_I8 *pValue);

static EAS_RESULT
RTTTL_PeekNextChar(EAS_HW_DATA_HANDLE hwInstData, S_RTTTL_DATA *pData, EAS_I8 *pValue);
...
static EAS_RESULT RTTTL_Prepare(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData) {
    ...
    if ((result = RTTTL_ParseHeader(pEASData, pData,
                                    (EAS_BOOL)(pData->metadata.callback != NULL))) != EAS_SUCCESS) {
...
static EAS_RESULT RTTTL_Event(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_INT parserMode) {
    ...
    for (;;) {
        ...
        if (c == 'b') {
            ...
            if ((result = RTTTL_GetOctave(pEASData->hwInstData, pData, &pData->octave)) !=
                EAS_SUCCESS)
                return result;
        }
            ...
            if (note != RTTTL_REST) {
                ...
                if (parserMode == eParserModePlay)
                    VMStartNote(pEASData->pVoiceMgr, pData->pSynth, RTTTL_CHANNEL, pData->note,
                                RTTTL_VELOCITY);
                ...
                switch (pData->style) {
                    ...
                    default: { /* dpp: EAS_ReportEx(_EAS_SEVERITY_WARNING, "RTTTL_Event: Unexpected style type %c\n", pData->style); */ }
                        break;
                }
...
static EAS_RESULT RTTTL_ParseHeader(S_EAS_DATA *pEASData, S_RTTTL_DATA *pData, EAS_BOOL metaData) {
    ...
    if ((result = EAS_HWFilePos(pEASData->hwInstData, pData->fileHandle, &pData->repeatOffset)) !=
        EAS_SUCCESS)
        return result;
...
RTTTL_PeekNextChar(EAS_HW_DATA_HANDLE hwInstData, S_RTTTL_DATA *pData, EAS_I8 *pValue) {
    ...
    *pValue = 0;
    for (;;) {
        ...
        if (!pData->dataByte) {
            if ((result = EAS_HWGetByte(hwInstData, pData->fileHandle, &pData->dataByte)) !=
                EAS_SUCCESS)
                return result;
