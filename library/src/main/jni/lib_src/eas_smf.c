...
/* local prototypes */
static EAS_RESULT
SMF_GetVarLenData(EAS_HW_DATA_HANDLE hwInstData, EAS_FILE_HANDLE fileHandle, EAS_U32 *pData);

static EAS_RESULT
SMF_ParseMetaEvent(S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream);

static EAS_RESULT
SMF_ParseSysEx(S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream, EAS_U8 f0,
               EAS_INT parserMode);

static EAS_RESULT
SMF_ParseEvent(S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream,
               EAS_INT parserMode);
...
*/
EAS_RESULT
SMF_CheckFileType(S_EAS_DATA *pEASData, EAS_FILE_HANDLE fileHandle, EAS_VOID_PTR *ppHandle,
                  EAS_I32 offset) {
    S_SMF_DATA *pSMFData;
    EAS_RESULT result;
...
EAS_RESULT SMF_Event(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_INT parserMode) {
    ...
    if ((result = SMF_ParseEvent(pEASData, pSMFData, pSMFData->nextStream, parserMode)) !=
        EAS_SUCCESS) {
        ...
        if ((result = SMF_GetDeltaTime(pEASData->hwInstData, pSMFData->nextStream)) !=
            EAS_SUCCESS) {
            /* check for unexpected end-of-file */
            if (result != EAS_EOF)
                return result;
...
EAS_RESULT SMF_Close(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData) {
    ...
    for (i = 0; i < pSMFData->numStreams; i++) {
        if (pSMFData->streams[i].fileHandle != NULL) {
            if ((result = EAS_HWCloseFile(pEASData->hwInstData, pSMFData->streams[i].fileHandle)) !=
                EAS_SUCCESS)
                return result;
        }
    }
...
EAS_RESULT SMF_Reset(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData) {
    ...
    for (i = 0; i < pSMFData->numStreams; i++) {
        ...
        if ((result = EAS_HWFileSeek(pEASData->hwInstData, pSMFData->streams[i].fileHandle,
                                     pSMFData->streams[i].startFilePos)) != EAS_SUCCESS)
            return result;
...
SMF_ParseMetaEvent(S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream) {
    ...
    /* get the length */
    if ((result = SMF_GetVarLenData(pEASData->hwInstData, pSMFStream->fileHandle, &len)) !=
        EAS_SUCCESS)
        return result;
    ...
    if (c == SMF_META_END_OF_TRACK) {
        ...
        while (len) {
            len--;
            if ((result = EAS_HWGetByte(pEASData->hwInstData, pSMFStream->fileHandle, &c)) !=
                EAS_SUCCESS)
                return result;
            temp = (temp << 8) | c;
        }
        ...
        if (metaType != EAS_METADATA_UNKNOWN) {
            ...
            if ((EAS_I32) len < readLen)
                readLen = (EAS_I32) len;
            if ((result = EAS_HWReadFile(pEASData->hwInstData, pSMFStream->fileHandle,
                                         pSMFData->metadata.buffer, readLen, &readLen)) !=
                EAS_SUCCESS)
                return result;
            pSMFData->metadata.buffer[readLen] = 0;
            pSMFData->metadata.callback(metaType, pSMFData->metadata.buffer,
                                        pSMFData->metadata.pUserData);
        }
    }
...
SMF_ParseSysEx(S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream, EAS_U8 f0,
    ...
    if (f0 == 0xf0) {
        if ((result = EAS_ParseMIDIStream(pEASData, pSMFData->pSynth, &pSMFStream->midiStream, f0,
                                          parserMode)) != EAS_SUCCESS)
            return result;
    }
    ...
    while (len) {
        ...
        if ((result = EAS_HWGetByte(pEASData->hwInstData, pSMFStream->fileHandle, &c)) !=
            EAS_SUCCESS)
            return result;
        if ((result = EAS_ParseMIDIStream(pEASData, pSMFData->pSynth, &pSMFStream->midiStream, c,
                                          parserMode)) != EAS_SUCCESS)
            return result;
    ...
    if (c == 0xff) {
        ...
        while (pSMFStream->midiStream.pending) {
            if ((result = EAS_HWGetByte(pEASData->hwInstData, pSMFStream->fileHandle, &c)) !=
                EAS_SUCCESS)
                return result;
            if ((result = EAS_ParseMIDIStream(pEASData, pSMFData->pSynth, &pSMFStream->midiStream,
                                              c, parserMode)) != EAS_SUCCESS)
                return result;
        }
...
EAS_RESULT SMF_ParseHeader(EAS_HW_DATA_HANDLE hwInstData, S_SMF_DATA *pSMFData) {
    ...
    if ((result = EAS_HWFileSeek(hwInstData, pSMFData->fileHandle,
                                 pSMFData->fileOffset + SMF_OFS_HEADER_SIZE)) != EAS_SUCCESS)
        goto ReadError;
    if ((result = EAS_HWGetDWord(hwInstData, pSMFData->fileHandle, &chunkSize, EAS_TRUE)) !=
        EAS_SUCCESS)
        goto ReadError;

    /* determine the number of tracks */
    if ((result = EAS_HWFileSeek(hwInstData, pSMFData->fileHandle,
                                 pSMFData->fileOffset + SMF_OFS_NUM_TRACKS)) != EAS_SUCCESS)
        goto ReadError;
    if ((result = EAS_HWGetWord(hwInstData, pSMFData->fileHandle, &numStreams, EAS_TRUE)) !=
        EAS_SUCCESS)
        goto ReadError;
    ...
    if ((result = EAS_HWGetWord(hwInstData, pSMFData->fileHandle, &division, EAS_TRUE)) !=
        EAS_SUCCESS)
        goto ReadError;
    ...
    if (!division || division &
                     0x8000) { /* dpp: EAS_ReportEx(_EAS_SEVERITY_WARNING,"No support for SMPTE code timebase\n"); */ }
    else
    ...
    for (i = 0; i < pSMFData->numStreams; i++) {
        ...
        for (;;) {
            ...
            if ((result = EAS_HWFileSeek(hwInstData, pSMFData->fileHandle, (EAS_I32) chunkStart)) !=
                EAS_SUCCESS)
                goto ReadError;

            /* read the chunk identifier */
            if ((result = EAS_HWGetDWord(hwInstData, pSMFData->fileHandle, &temp, EAS_TRUE)) !=
                EAS_SUCCESS)
                goto ReadError;
        ...
        if (i < (pSMFData->numStreams - 1)) {
            if ((result = EAS_HWDupHandle(hwInstData, pSMFData->streams[i].fileHandle,
                                          &pSMFData->fileHandle)) != EAS_SUCCESS)
                goto ReadError;
        }
    }
