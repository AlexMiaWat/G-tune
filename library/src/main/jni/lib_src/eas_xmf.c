...
/* local prototypes */
static EAS_RESULT
XMF_CheckFileType(S_EAS_DATA *pEASData, EAS_FILE_HANDLE fileHandle, EAS_VOID_PTR *ppHandle,
                  EAS_I32 offset);
...
static EAS_RESULT
XMF_SetData(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_I32 param, EAS_I32 value);

static EAS_RESULT
XMF_GetData(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_I32 param, EAS_I32 *pValue);
...
XMF_ReadNode(EAS_HW_DATA_HANDLE hwInstData, S_XMF_DATA *pXMFData, EAS_I32 nodeOffset,
             EAS_I32 endOffset, EAS_I32 *pLength, EAS_I32 depth);

static EAS_RESULT
XMF_ReadVLQ(EAS_HW_DATA_HANDLE hwInstData, EAS_FILE_HANDLE fileHandle, EAS_U32 *remainingBytes,
            EAS_I32 *value);
...
XMF_CheckFileType(S_EAS_DATA *pEASData, EAS_FILE_HANDLE fileHandle, EAS_VOID_PTR *ppHandle,
    ...
    *ppHandle = NULL;
    ...
    if ((result = EAS_HWFileSeek(pEASData->hwInstData, fileHandle, pXMFData->midiOffset)) !=
        EAS_SUCCESS) {
        EAS_HWFree(pEASData->hwInstData, pXMFData);
        return result;
    }
    if ((result = SMF_CheckFileType(pEASData, fileHandle, &pXMFData->pSMFData,
                                    pXMFData->midiOffset)) != EAS_SUCCESS) {
        EAS_HWFree(pEASData->hwInstData, pXMFData);
        return result;
    }
    *ppHandle = pXMFData;
...
static EAS_RESULT XMF_Prepare(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData) {
    ...
    if (pXMFData->dlsOffset != 0) {
        if ((result = DLSParser(pEASData->hwInstData, pXMFData->fileHandle, pXMFData->dlsOffset,
                                &pXMFData->pDLS)) != EAS_SUCCESS) {
...
XMF_GetData(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_I32 param, EAS_I32 *pValue) {
    ...
    if ((result = SMF_GetData(pEASData, ((S_XMF_DATA *) pInstData)->pSMFData, param, pValue)) !=
        EAS_SUCCESS)
        return result;
...
static EAS_RESULT XMF_FindFileContents(EAS_HW_DATA_HANDLE hwInstData, S_XMF_DATA *pXMFData) {
    ...
    if ((result = XMF_ReadVLQ(hwInstData, pXMFData->fileHandle, &remainingBytes, &value)) !=
        EAS_SUCCESS)
        return result;
    ...
    if ((result = XMF_ReadVLQ(hwInstData, pXMFData->fileHandle, &remainingBytes, &treeStart)) !=
        EAS_SUCCESS)
        return result;
    if (treeStart < 0)
        return EAS_ERROR_FILE_FORMAT;

    if ((result = XMF_ReadVLQ(hwInstData, pXMFData->fileHandle, &remainingBytes, &treeEnd)) !=
        EAS_SUCCESS)
        return result;
    if (treeEnd < treeStart || treeEnd >= fileLength)
        return EAS_ERROR_FILE_FORMAT;

    if ((result = XMF_ReadNode(hwInstData, pXMFData, treeStart, treeEnd + 1, &length,
                               node_depth)) != EAS_SUCCESS)
        return result;
    ...
    if ((pXMFData->dlsOffset > 0) && (pXMFData->midiOffset <
                                      pXMFData->dlsOffset)) { /* dpp: EAS_ReportEx(_EAS_SEVERITY_WARNING, "DLS data must precede SMF data in Mobile XMF file\n"); */ }
...
XMF_ReadNode(EAS_HW_DATA_HANDLE hwInstData, S_XMF_DATA *pXMFData, EAS_I32 nodeOffset,
    ...
    /* get node length */
    if ((result = XMF_ReadVLQ(hwInstData, pXMFData->fileHandle, &remainingInlineBytes, pLength)) !=
        EAS_SUCCESS)
        return result;
    ...
    if ((result = XMF_ReadVLQ(hwInstData, pXMFData->fileHandle, &remainingInlineBytes,
                              &numItems)) != EAS_SUCCESS)
        return result;
    if (numItems < 0)
        return EAS_ERROR_FILE_FORMAT;
    ...
    if ((result = XMF_ReadVLQ(hwInstData, pXMFData->fileHandle, &remainingInlineBytes,
                              &headerLength)) != EAS_SUCCESS)
        return result;
    if (headerLength < 0 || headerLength > *pLength)
        return EAS_ERROR_FILE_FORMAT;

    /* get metadata length */
    if ((result = XMF_ReadVLQ(hwInstData, pXMFData->fileHandle, &remainingInlineBytes, &length)) !=
        EAS_SUCCESS)
        return result;
    if (length < 0)
        return EAS_ERROR_FILE_FORMAT;
    ...
    if ((result = EAS_HWFileSeek(hwInstData, pXMFData->fileHandle, nodeOffset + headerLength)) !=
        EAS_SUCCESS)
        return result;
    remainingInlineBytes = endOffset - (nodeOffset + headerLength);

    /* get reference type */
    if ((result = XMF_ReadVLQ(hwInstData, pXMFData->fileHandle, &remainingInlineBytes, &refType)) !=
        EAS_SUCCESS)
        return result;
    ...
    if (numItems == 0) {
        ...
        /* get the chunk type */
        if ((result = EAS_HWGetDWord(hwInstData, pXMFData->fileHandle, &chunkType, EAS_TRUE)) !=
            EAS_SUCCESS)
            return result;
        ...
        for (; numItems > 0; numItems--) {
            /* process this item */
            if ((result = XMF_ReadNode(hwInstData, pXMFData, offset, endOffset, &length,
                                       depth + 1)) != EAS_SUCCESS)
                return result;
