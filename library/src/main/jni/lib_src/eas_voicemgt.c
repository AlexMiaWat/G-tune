...
void VMReleaseVoice(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, EAS_I32 voiceNum) {
    ...
    /* release this voice */
    GetSynthPtr(voiceNum)->pfReleaseVoice(pVoiceMgr, pSynth, &pVoiceMgr->voices[voiceNum],
                                          GetAdjustedVoiceNum(voiceNum));
    pVoice->voiceState = eVoiceStateRelease;
}
    ...
    for (voiceNum = 0; voiceNum < MAX_SYNTH_VOICES; voiceNum++) {
        ...
        if (pVoice->voiceState != eVoiceStateFree) {
            if (((pVoice->voiceState != eVoiceStateStolen) && (channel == pVoice->channel)) ||
                ...
                GetSynthPtr(voiceNum)->pfMuteVoice(pVoiceMgr, pSynth, pVoice,
                                                   GetAdjustedVoiceNum(voiceNum));
                pVoice->voiceState = eVoiceStateMuting;
            }
        }
            ...
            if (pVoiceMgr->voices[voiceNum].voiceState == eVoiceStateStolen) {
                ...
                */
                ...
                if (pSynth->channels[channel].channelFlags & CHANNEL_FLAG_SUSTAIN_PEDAL) {
                    GetSynthPtr(voiceNum)->pfSustainPedal(pVoiceMgr, pSynth,
                                                          &pVoiceMgr->voices[voiceNum],
                                                          &pSynth->channels[channel],
                                                          GetAdjustedVoiceNum(voiceNum));
                }
 ...
 *----------------------------------------------------------------------------
*/
static void VMStolenVoice(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, EAS_I32 voiceNum, EAS_U8 channel,
                          EAS_U8 note, EAS_U8 velocity, EAS_U16 regionIndex) {
    S_SYNTH_VOICE *pVoice = &pVoiceMgr->voices[voiceNum];
    ...
    GetSynthPtr(voiceNum)->pfMuteVoice(pVoiceMgr, pVoiceMgr->pSynth[GET_VSYNTH(pVoice->channel)],
                                       &pVoiceMgr->voices[voiceNum], GetAdjustedVoiceNum(voiceNum));
...
static EAS_BOOL VMRetargetStolenVoice(S_VOICE_MGR *pVoiceMgr, EAS_I32 voiceNum) {
    ...
    GetSynthPtr(voiceNum)->pfStartVoice(pVoiceMgr, pNextSynth, &pVoiceMgr->voices[voiceNum],
                                        GetAdjustedVoiceNum(voiceNum), pVoice->regionIndex);
        ...
        if (pVoiceMgr->voices[voiceNum].voiceState != eVoiceStateStolen) {
            ...
            if (channel == pVoiceMgr->voices[voiceNum].channel) {
                ...
                if (keyGroup == (pRegion->keyGroupAndFlags & 0x0f00)) {
                    ...
                    if (pVoiceMgr->voices[voiceNum].voiceFlags &
                        VOICE_FLAG_NO_SAMPLES_SYNTHESIZED_YET)
...
*/
EAS_BOOL
VMCheckPolyphonyLimiting(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, EAS_U8 channel, EAS_U8 note,
                         EAS_U8 velocity, EAS_U16 regionIndex, EAS_I32 lowVoice,
                         EAS_I32 highVoice) {
    EAS_INT voiceNum;
    ...
    for (voiceNum = lowVoice; voiceNum <= highVoice; voiceNum++) {
        ...
        if (pVoiceMgr->voices[voiceNum].voiceState != eVoiceStateStolen) {
            ...
            if ((channel == pVoiceMgr->voices[voiceNum].channel) &&
                (note == pVoiceMgr->voices[voiceNum].note)) {
            ...
            /* same channel and note ? */
            if ((channel == pVoiceMgr->voices[voiceNum].nextChannel) &&
                (note == pVoiceMgr->voices[voiceNum].nextNote)) {
                numVoicesPlayingNote++;
            }
        }
    }
...
*/
void
VMStartVoice(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, EAS_U8 channel, EAS_U8 note, EAS_U8 velocity,
             EAS_U16 regionIndex) {
    const S_REGION *pRegion;
    S_SYNTH_CHANNEL *pChannel;
    EAS_INT voiceNum;
    ...
    if (pSynth->poolCount[pChannel->pool] != 0) {
        ...
        if ((pRegion->keyGroupAndFlags & REGION_FLAG_NON_SELF_EXCLUSIVE) == 0) {
            if (VMCheckPolyphonyLimiting(pVoiceMgr, pSynth, channel, note, velocity, regionIndex,
                                         lowVoice, highVoice) == EAS_TRUE)
                return;
        }
    }
    ...
    if ((pVoiceMgr->activeVoices < pVoiceMgr->maxPolyphony) &&
        ...
        /*lint -e{522} return not used at this time */
        GetSynthPtr(voiceNum)->pfStartVoice(pVoiceMgr, pSynth, &pVoiceMgr->voices[voiceNum],
                                            GetAdjustedVoiceNum(voiceNum), regionIndex);
        return;
    }

    /* no free voices, we have to steal one using appropriate algorithm */
    if (VMStealVoice(pVoiceMgr, pSynth, &voiceNum, channel, note, lowVoice, highVoice) ==
        EAS_SUCCESS)
...
*/
void
VMStartNote(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, EAS_U8 channel, EAS_U8 note, EAS_U8 velocity) {
    S_SYNTH_CHANNEL *pChannel;
    EAS_U16 regionIndex;
    ...
    if (regionIndex & FLAG_RGN_IDX_DLS_SYNTH) {
        ...
        for (;;) {
            ...
            if (((adjustedNote >= pDLSRegion->wtRegion.region.rangeLow) &&
                 (adjustedNote <= pDLSRegion->wtRegion.region.rangeHigh)) &&
...
void
VMStopNote(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, EAS_U8 channel, EAS_U8 note, EAS_U8 velocity) {
    S_SYNTH_CHANNEL *pChannel;
    EAS_INT voiceNum;
    ...
    for (voiceNum = 0; voiceNum < MAX_SYNTH_VOICES; voiceNum++) {
        ...
        else if ((channel == pVoiceMgr->voices[voiceNum].nextChannel) &&
                 (note == pVoiceMgr->voices[voiceNum].nextNote)) {
 ...
 *----------------------------------------------------------------------------
*/
EAS_RESULT VMFindAvailableVoice(S_VOICE_MGR *pVoiceMgr, EAS_INT *pVoiceNumber, EAS_I32 lowVoice,
                                EAS_I32 highVoice) {
    EAS_INT voiceNum;
...
*/
EAS_RESULT
VMStealVoice(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, EAS_INT *pVoiceNumber, EAS_U8 channel,
             EAS_U8 note, EAS_I32 lowVoice, EAS_I32 highVoice) {
    ...
    for (voiceNum = lowVoice; voiceNum <= highVoice; voiceNum++) {
        ...
        if ((pCurrVoice->voiceState == eVoiceStateStolen) ||
            (pCurrVoice->voiceFlags & VOICE_FLAG_NO_SAMPLES_SYNTHESIZED_YET)) {
        ...
        if (pSynth->synthFlags & SYNTH_FLAG_SP_MIDI_ON) {
            ...
            if (pSynth->poolCount[pChannel->pool] >= pSynth->poolAlloc[pChannel->pool])
                currentPriority +=
                        (pSynth->poolCount[pChannel->pool] - pSynth->poolAlloc[pChannel->pool] + 1)
                                << CHANNEL_POLY_STEAL_WEIGHT;
 ...
 *----------------------------------------------------------------------------
*/
void VMControlChange(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, EAS_U8 channel, EAS_U8 controller,
                     EAS_U8 value) {
    S_SYNTH_CHANNEL *pChannel;
...
*/
static EAS_RESULT
VMFindProgram(const S_EAS *pEAS, EAS_U32 bank, EAS_U8 programNum, EAS_U16 *pRegionIndex) {
    EAS_U32 locale;
    const S_PROGRAM *p;
    EAS_U16 i;
    EAS_U16 regionIndex;
...
*/
static EAS_RESULT
VMFindDLSProgram(const S_DLS *pDLS, EAS_U32 bank, EAS_U8 programNum, EAS_U16 *pRegionIndex) {
    EAS_U32 locale;
    const S_PROGRAM *p;
    EAS_U16 i;
...
void VMProgramChange(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, EAS_U8 channel, EAS_U8 program) {
        ...
        /* braces to support 'if' clause above */
    {
            ...
            /* fall back to default bank */
        {
            ...
            {
                if (VMFindProgram(pSynth->pEAS, bank, 0, &regionIndex) != EAS_SUCCESS) { /* dpp: EAS_ReportEx(_EAS_SEVERITY_WARNING, "VMProgramChange: No program @ %03d:%03d:%03d\n",
...
EAS_I32 VMAddSamples(S_VOICE_MGR *pVoiceMgr, EAS_I32 *pMixBuffer, EAS_I32 numSamples) {
    ...
    for (voiceNum = 0; voiceNum < MAX_SYNTH_VOICES; voiceNum++) {
        ...
        if (pVoiceMgr->voices[voiceNum].voiceState != eVoiceStateFree) {
            done = GetSynthPtr(voiceNum)->pfUpdateVoice(pVoiceMgr, pSynth,
                                                        &pVoiceMgr->voices[voiceNum],
                                                        GetAdjustedVoiceNum(voiceNum), pMixBuffer,
                                                        numSamples);
            voicesRendered++;
 ...
 *----------------------------------------------------------------------------
*/
EAS_RESULT VMRender(S_VOICE_MGR *pVoiceMgr, EAS_I32 numSamples, EAS_I32 *pMixBuffer,
                    EAS_I32 *pVoicesRendered) {
    S_SYNTH *pSynth;
    EAS_INT i;
    EAS_INT channel;
    ...
    while (activeVoices > polyphonyCount) {
        ...
        for (i = 0; i < MAX_SYNTH_VOICES; i++) {
            ...
            if ((pVoice->voiceState == eVoiceStateStolen) ||
                ...
                /* include channel priority */
                currentPriority += pSynth->channels[GET_CHANNEL(pVoice->nextChannel)].pool
                        << CHANNEL_PRIORITY_STEAL_WEIGHT;
            } else {
                /* include age */
...
void VMMIDIShutdown(S_EAS_DATA *pEASData, S_SYNTH *pSynth) {
    ...
    if (pSynth->pDLS != NULL) {
        EAS_RESULT result;
        if ((result = DLSCleanup(pEASData->hwInstData, pSynth->pDLS)) !=
