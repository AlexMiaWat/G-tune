...
*/
static EAS_RESULT
NextChunk(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 *pPos, EAS_U32 *pChunkType, EAS_I32 *pSize);

static EAS_RESULT
Parse_ptbl(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, EAS_I32 wsmpPos, EAS_I32 wsmpSize);
...
static EAS_RESULT
Parse_data(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, EAS_I32 size, S_WSMP_DATA *p,
           EAS_SAMPLE *pSample, EAS_U32 sampleLen);
...
static EAS_RESULT
Parse_insh(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, EAS_U32 *pRgnCount, EAS_U32 *pLocale);

static EAS_RESULT
Parse_lrgn(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, EAS_I32 size, EAS_U16 artIndex,
           EAS_U32 numRegions);

static EAS_RESULT
Parse_rgn(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, EAS_I32 size, EAS_U16 artIndex);

static EAS_RESULT Parse_rgnh(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, S_DLS_REGION *pRgn);

static EAS_RESULT
Parse_lart(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, EAS_I32 size, S_DLS_ART_VALUES *pArt);
...
static void Convert_rgn(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_U16 regionIndex, EAS_U16 artIndex,
                        EAS_U16 waveIndex, S_WSMP_DATA *pWsmp);

static void
Convert_art(SDLS_SYNTHESIZER_DATA *pDLSData, const S_DLS_ART_VALUES *pDLSArt, EAS_U16 artIndex);
 ...
 *----------------------------------------------------------------------------
*/
EAS_RESULT DLSParser(EAS_HW_DATA_HANDLE hwInstData, EAS_FILE_HANDLE fileHandle, EAS_I32 offset,
                     EAS_DLSLIB_HANDLE *ppDLS) {
    EAS_RESULT result;
    ...
    *ppDLS = NULL;
    ...
    if ((result = EAS_HWFileSeek(dls.hwInstData, dls.fileHandle, offset)) != EAS_SUCCESS)
        return result;
    if ((result = EAS_HWReadFile(dls.hwInstData, dls.fileHandle, &chunk_type, sizeof(chunk_type),
                                 &size)) != EAS_SUCCESS)
        return result;
    ...
    if ((ptblSize == 0) ||
        (ptblSize > (EAS_I32) (DLS_MAX_WAVE_COUNT * sizeof(POOLCUE) + sizeof(POOLTABLE)))) {
    ...
    if (result == EAS_SUCCESS) {
        ...
        size = (EAS_I32) sizeof(S_EAS) + instSize + rgnPoolSize + artPoolSize + (2 * waveLenSize) +
               (EAS_I32) dls.wavePoolSize;
        if (size <= 0) {
            EAS_HWFree(dls.hwInstData, dls.wsmpData);
            return EAS_ERROR_FILE_FORMAT;
        }
...
NextChunk(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 *pPos, EAS_U32 *pChunkType, EAS_I32 *pSize) {
    ...
    /* read the chunk type */
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, pChunkType,
                                 EAS_TRUE)) != EAS_SUCCESS)
        return result;

    /* read the chunk size */
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, pSize, EAS_FALSE)) !=
        EAS_SUCCESS)
        return result;
...
*/
static EAS_RESULT
Parse_ptbl(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, EAS_I32 wtblPos, EAS_I32 wtblSize) {
    EAS_RESULT result;
    EAS_U32 temp;
    ...
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &temp, EAS_FALSE)) !=
        EAS_SUCCESS)
        return result;

    /* get the number of waves */
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &pDLSData->waveCount,
    ...
    if ((result = EAS_HWDupHandle(pDLSData->hwInstData, pDLSData->fileHandle, &tempFile)) !=
        EAS_SUCCESS)
        return result;
    ...
    for (waveIndex = 0; waveIndex < pDLSData->waveCount; waveIndex++) {
        ...
        if ((result = EAS_HWGetDWord(pDLSData->hwInstData, tempFile, &temp, EAS_FALSE)) !=
            EAS_SUCCESS)
            return result;
...
static EAS_RESULT Parse_wsmp(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, S_WSMP_DATA *p) {
    ...
    /* get structure size */
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &cbSize, EAS_FALSE)) !=
        EAS_SUCCESS)
        return result;

    /* get unity note */
    if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &wtemp, EAS_FALSE)) !=
        EAS_SUCCESS)
        return result;
    if (wtemp <= 127)
    ...
    /* get fine tune */
    if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &p->fineTune,
                                EAS_FALSE)) != EAS_SUCCESS)
        return result;

    /* get gain */
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &p->gain,
                                 EAS_FALSE)) != EAS_SUCCESS)
        return result;
    if (p->gain > 0) {
    ...
    /* option flags */
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &ltemp, EAS_FALSE)) !=
        EAS_SUCCESS)
        return result;
    ...
    if (ltemp) {
        ...
        if ((result = EAS_HWFileSeek(pDLSData->hwInstData, pDLSData->fileHandle,
                                     pos + (EAS_I32) cbSize)) != EAS_SUCCESS)
            return result;

        /* get structure size */
        if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &ltemp,
                                     EAS_FALSE)) != EAS_SUCCESS)
            return result;
        ...
        /* get loop start */
        if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &p->loopStart,
                                     EAS_FALSE)) != EAS_SUCCESS)
            return result;

        /* get loop length */
        if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &p->loopLength,
                                     EAS_FALSE)) != EAS_SUCCESS)
            return result;
...
static EAS_RESULT Parse_fmt(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, S_WSMP_DATA *p) {
    ...
    /* get sample rate */
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &p->sampleRate,
                                 EAS_FALSE)) != EAS_SUCCESS)
        return result;
    ...
    /* bits/sample */
    if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &p->bitsPerSample,
                                EAS_FALSE)) != EAS_SUCCESS)
        return result;
...
static EAS_RESULT
Parse_data(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, EAS_I32 size, S_WSMP_DATA *pWsmp,
           EAS_SAMPLE *pSample, EAS_U32 sampleLen) {
    EAS_RESULT result;
    EAS_U8 convBuf[SAMPLE_CONVERT_CHUNK_SIZE];
    EAS_I32 count = 0;
    ...
    while (size) {
        /* read a small chunk of data and convert it */
        count = (size < SAMPLE_CONVERT_CHUNK_SIZE ? size : SAMPLE_CONVERT_CHUNK_SIZE);
        if ((result = EAS_HWReadFile(pDLSData->hwInstData, pDLSData->fileHandle, convBuf, count,
                                     &count)) != EAS_SUCCESS) {
            return result;
        }
        size -= count;
...
Parse_insh(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, EAS_U32 *pRgnCount, EAS_U32 *pLocale) {
    ...
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, pRgnCount,
                                 EAS_FALSE)) != EAS_SUCCESS)
        return result;
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &bank, EAS_FALSE)) !=
        EAS_SUCCESS)
        return result;
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &program,
                                 EAS_FALSE)) != EAS_SUCCESS)
        return result;
...
Parse_lrgn(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, EAS_I32 size, EAS_U16 artIndex,
    ...
    if ((pDLSData->pDLS != NULL) && (regionCount > 0))
        pDLSData->pDLS->pDLSRegions[pDLSData->regionCount -
                                    1].wtRegion.region.keyGroupAndFlags |= REGION_FLAG_LAST_REGION;
...
Parse_rgn(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, EAS_I32 size, EAS_U16 artIndex) {
    ...
    if (pDLSData->pDLS) {
        ...
        /* parse region header */
        if ((result = Parse_rgnh(pDLSData, rgnhPos,
                                 &pDLSData->pDLS->pDLSRegions[regionIndex & REGION_INDEX_MASK])) !=
            EAS_SUCCESS)
            return result;
        ...
        if (pWsmp->loopLength != 0) {
            ...
            if (sampleLen < sizeof(EAS_SAMPLE)
                || (pWsmp->loopStart + pWsmp->loopLength) * sizeof(EAS_SAMPLE) >
                   sampleLen - sizeof(EAS_SAMPLE)) {
                return EAS_FAILURE;
            }
        }
    }
...
static EAS_RESULT Parse_rgnh(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, S_DLS_REGION *pRgn) {
    ...
    /* get the key range */
    if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &lowKey, EAS_FALSE)) !=
        EAS_SUCCESS)
        return result;
    if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &highKey, EAS_FALSE)) !=
        EAS_SUCCESS)
        return result;
    ...
    if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &lowVel, EAS_FALSE)) !=
        EAS_SUCCESS)
        return result;
    if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &highVel, EAS_FALSE)) !=
        EAS_SUCCESS)
        return result;
    ...
    if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &optionFlags,
                                EAS_FALSE)) != EAS_SUCCESS)
        return result;

    /* get the key group */
    if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &keyGroup,
                                EAS_FALSE)) != EAS_SUCCESS)
        return result;
...
static EAS_RESULT Parse_art(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, S_DLS_ART_VALUES *pArt) {
    ...
    /* get the structure size */
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &structSize,
                                 EAS_FALSE)) != EAS_SUCCESS)
        return result;
    pos += (EAS_I32) structSize;

    /* get the number of connections */
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &numConnections,
                                 EAS_FALSE)) != EAS_SUCCESS)
        return result;
    ...
    while (numConnections) {
        ...
        if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &source,
                                    EAS_FALSE)) != EAS_SUCCESS)
            return result;
        if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &control,
                                    EAS_FALSE)) != EAS_SUCCESS)
            return result;
        if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &destination,
                                    EAS_FALSE)) != EAS_SUCCESS)
            return result;
        if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &transform,
                                    EAS_FALSE)) != EAS_SUCCESS)
            return result;
        if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &scale,
                                     EAS_FALSE)) != EAS_SUCCESS)
            return result;
        ...
        if (i ==
            PARAM_TABLE_SIZE) { /* dpp: EAS_ReportEx(_EAS_SEVERITY_WARNING, "WARN: Unsupported parameter in DLS file\n"); */ }
    }
...
static EAS_RESULT Parse_wlnk(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 pos, EAS_U32 *pWaveIndex) {
    ...
    if ((result = EAS_HWFileSeek(pDLSData->hwInstData, pDLSData->fileHandle, pos + 8)) !=
        EAS_SUCCESS)
        return result;
...
static EAS_RESULT ReadDLSID(SDLS_SYNTHESIZER_DATA *pDLSData, DLSID *pDLSID) {
    ...
    if ((result = EAS_HWGetDWord(pDLSData->hwInstData, pDLSData->fileHandle, &pDLSID->Data1,
                                 EAS_FALSE)) != EAS_SUCCESS)
        return result;
    if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &pDLSID->Data2,
                                EAS_FALSE)) != EAS_SUCCESS)
        return result;
    if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &pDLSID->Data3,
                                EAS_FALSE)) != EAS_SUCCESS)
        return result;
    return EAS_HWReadFile(pDLSData->hwInstData, pDLSData->fileHandle, pDLSID->Data4,
                          sizeof(pDLSID->Data4), &n);
}
...
static EAS_RESULT Parse_cdl(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_I32 size, EAS_U32 *pValue) {
    ...
    *pValue = 0;
    ...
    while (size) {
        /* read the opcode */
        if ((result = EAS_HWGetWord(pDLSData->hwInstData, pDLSData->fileHandle, &opcode,
                                    EAS_FALSE)) != EAS_SUCCESS)
            return result;
        ...
        if (opcode <= DLS_CDL_EQ) {
            ...
            if ((result = ReadDLSID(pDLSData, &dlsid)) != EAS_SUCCESS)
                return result;
            x = QueryGUID(&dlsid, &y);
        } else { /* dpp: EAS_ReportEx(_EAS_SEVERITY_WARNING, "Unsupported opcode %d in DLS file\n", opcode); */ }
...
static void Convert_rgn(SDLS_SYNTHESIZER_DATA *pDLSData, EAS_U16 regionIndex, EAS_U16 artIndex,
    ...
    pRgn->wtRegion.tuning =
            pWsmp->fineTune - (pWsmp->unityNote * 100) + ConvertSampleRate(pWsmp->sampleRate);
    if (pWsmp->loopLength != 0)
