...
/* local prototypes */
static EAS_RESULT
EAS_ParseEvents(S_EAS_DATA *pEASData, S_EAS_STREAM *pStream, EAS_U32 endTime, EAS_INT parseMode);
...
*/
EAS_RESULT
EAS_SetStreamParameter(S_EAS_DATA *pEASData, EAS_HANDLE pStream, EAS_I32 param, EAS_I32 value) {
...
*/
EAS_RESULT
EAS_GetStreamParameter(S_EAS_DATA *pEASData, EAS_HANDLE pStream, EAS_I32 param, EAS_I32 *pValue) {
    S_FILE_PARSER_INTERFACE *pParserModule;
...
EAS_IntSetStrmParam(S_EAS_DATA *pEASData, EAS_HANDLE pStream, EAS_INT param, EAS_I32 value) {
    ...
    if (EAS_GetStreamParameter(pEASData, pStream, PARSER_DATA_SYNTH_HANDLE, (EAS_I32 *) &pSynth) !=
        EAS_SUCCESS)
    ...
    switch (param) {
        ...
        default: { /* dpp: EAS_ReportEx(_EAS_SEVERITY_ERROR, "Invalid paramter %d in call to EAS_IntSetStrmParam", param); */ }
            return EAS_ERROR_INVALID_PARAMETER;
    }
...
*/
static void
EAS_InitStream(S_EAS_STREAM *pStream, EAS_VOID_PTR pParserModule, EAS_VOID_PTR streamHandle) {
    ...
    *ppEASData = NULL;
    ...
    for (module = 0; module < NUM_EFFECTS_MODULES; module++) {
        ...
        if (pEASData->effectsModules[module].effect != NULL) {
            if ((result = (*pEASData->effectsModules[module].effect->pfInit)(pEASData,
                                                                             &pEASData->effectsModules[module].effectData)) !=
                EAS_SUCCESS) {
...
EAS_PUBLIC EAS_RESULT EAS_Shutdown(EAS_DATA_HANDLE pEASData) {
    ...
    for (i = 0; i < MAX_NUMBER_STREAMS; i++) {
        if (pEASData->streams[i].pParserModule && pEASData->streams[i].handle) {
            if ((result = (*((S_FILE_PARSER_INTERFACE *) (pEASData->streams[i].pParserModule))->pfClose)(
                    pEASData, pEASData->streams[i].handle)) != EAS_SUCCESS) {
    ...
    for (i = 0; i < NUM_EFFECTS_MODULES; i++) {
        if (pEASData->effectsModules[i].effect) {
            if ((result = (*pEASData->effectsModules[i].effect->pfShutdown)(pEASData,
                                                                            pEASData->effectsModules[i].effectData)) !=
                EAS_SUCCESS) {
...
EAS_OpenFile(EAS_DATA_HANDLE pEASData, EAS_FILE_LOCATOR locator, EAS_HANDLE *ppStream) {
    EAS_RESULT result;
    ...
    *ppStream = NULL;
    ...
    for (moduleNum = 0;; moduleNum++) {
        ...
        if ((result = (*pParserModule->pfCheckFileType)(pEASData, fileHandle, &streamHandle, 0L)) !=
            EAS_SUCCESS) {
...
*/
EAS_PUBLIC EAS_RESULT
EAS_Render(EAS_DATA_HANDLE pEASData, EAS_PCM *pOut, EAS_I32 numRequested, EAS_I32 *pNumGenerated) {
    S_FILE_PARSER_INTERFACE *pParserModule;
    EAS_RESULT result;
    ...
    *pNumGenerated = 0;
    ...
    for (streamNum = 0; streamNum < MAX_NUMBER_STREAMS; streamNum++) {
        ...
        if (pEASData->streams[streamNum].pParserModule) {
            ...
            /* get current state */
            if ((result = (*pParserModule->pfState)(pEASData, pEASData->streams[streamNum].handle,
                                                    &parserState)) != EAS_SUCCESS)
                return result;
            ...
            if ((pEASData->streams[streamNum].streamFlags & STREAM_FLAGS_PARSED) == 0)
                if ((result = EAS_ParseEvents(pEASData, &pEASData->streams[streamNum],
                                              pEASData->streams[streamNum].time +
                                              pEASData->streams[streamNum].frameLength,
                                              eParserModePlay)) != EAS_SUCCESS)
                    return result;
            ...
            if (pEASData->streams[streamNum].repeatCount) {
                ...
                if ((result = (*pParserModule->pfState)(pEASData,
                                                        pEASData->streams[streamNum].handle,
                                                        &parserState)) != EAS_SUCCESS)
                    return result;
                if (parserState == EAS_STATE_STOPPED) {
                    ...
                    /* reset the parser */
                    if ((result = (*pParserModule->pfReset)(pEASData,
                                                            pEASData->streams[streamNum].handle)) !=
                        EAS_SUCCESS)
                        return result;
    ...
    /* render audio */
    if ((result = VMRender(pEASData->pVoiceMgr, BUFFER_SIZE_IN_MONO_SAMPLES, pEASData->pMixBuffer,
                           &voicesRendered)) != EAS_SUCCESS) {
...
*/
static EAS_RESULT
EAS_ParseEvents(S_EAS_DATA *pEASData, EAS_HANDLE pStream, EAS_U32 endTime, EAS_INT parseMode) {
    S_FILE_PARSER_INTERFACE *pParserModule;
    EAS_RESULT result;
    EAS_I32 parserState;
    EAS_BOOL done;
    ...
    if (pParserModule->pfTime == NULL) {
        /* check state */
        if ((result = (*pParserModule->pfState)(pEASData, pStream->handle, &parserState)) !=
            EAS_SUCCESS)
            return result;
    ...
    while (!done) {
        ...
        if (pParserModule->pfTime) {
            if ((result = (*pParserModule->pfTime)(pEASData, pStream->handle, &time)) !=
                EAS_SUCCESS)
                return result;
...
*/
EAS_PUBLIC EAS_RESULT
EAS_ParseMetaData(EAS_DATA_HANDLE pEASData, EAS_HANDLE pStream, EAS_I32 *playLength) {
    S_FILE_PARSER_INTERFACE *pParserModule;
    EAS_RESULT result;
    EAS_STATE state;
    ...
    pStream->time = 0;
    VMInitWorkload(pEASData->pVoiceMgr);
    if ((result = EAS_ParseEvents(pEASData, pStream, 0x7fffffff, eParserModeMetaData)) !=
        EAS_SUCCESS)
        return result;
...
*/
EAS_PUBLIC EAS_RESULT
EAS_Locate(EAS_DATA_HANDLE pEASData, EAS_HANDLE pStream, EAS_I32 milliseconds, EAS_BOOL offset) {
    S_FILE_PARSER_INTERFACE *pParserModule;
    EAS_RESULT result;
    ...
    if (pParserModule->pfLocate != NULL) {
        ...
        result = pParserModule->pfLocate(pEASData, pStream->handle, (EAS_I32) requestedTime,
                                         &parserLocate);
        if (!parserLocate) {
            if (result == EAS_SUCCESS)
    ...
    if (((state == EAS_STATE_PAUSING) || (state == EAS_STATE_PAUSED)) &&
        ((pStream->streamFlags & STREAM_FLAGS_RESUME) == 0))
    ...
    if ((result = EAS_ParseEvents(pEASData, pStream, requestedTime << 8, eParserModeLocate)) !=
        EAS_SUCCESS)
        return result;
...
EAS_PUBLIC EAS_RESULT
EAS_GetLocation(EAS_DATA_HANDLE pEASData, EAS_HANDLE pStream, EAS_I32 *pTime) {
    if (!EAS_StreamReady(pEASData, pStream))
        return EAS_ERROR_NOT_VALID_IN_THIS_STATE;
...
*/
EAS_PUBLIC EAS_RESULT
EAS_SetParameter(EAS_DATA_HANDLE pEASData, EAS_I32 module, EAS_I32 param, EAS_I32 value) {
