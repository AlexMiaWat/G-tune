...
/* function prototypes */
extern EAS_RESULT
EAS_IntSetStrmParam(S_EAS_DATA *pEASData, EAS_HANDLE pStream, EAS_INT param, EAS_I32 value);

extern EAS_RESULT
EAS_OpenJETStream(EAS_DATA_HANDLE pEASData, EAS_FILE_HANDLE fileHandle, EAS_I32 offset,
                  EAS_HANDLE *ppStream);

extern EAS_RESULT
DLSParser(EAS_HW_DATA_HANDLE hwInstData, EAS_FILE_HANDLE fileHandle, EAS_I32 offset,
...
*/
EAS_INLINE void
JET_WriteQueue(EAS_U32 *pEventQueue, EAS_U8 *pWriteIndex, EAS_U8 readIndex, EAS_U8 queueSize,
               EAS_U32 event) {
    EAS_U8 temp;
...
*/
EAS_INLINE EAS_BOOL
JET_ReadQueue (EAS_U32
*pEventQueue,
EAS_U8 *pReadIndex, EAS_U8
writeIndex,
EAS_U8 queueSize, EAS_U32
*pEvent)
{
...
static EAS_RESULT JET_PrepareSegment(EAS_DATA_HANDLE easHandle, EAS_I32 queueNum) {
    ...
    if (p->libNum >= 0) {
        result = EAS_IntSetStrmParam(easHandle, p->streamHandle,
                                     ...
                                     (EAS_I32) easHandle->jetHandle->libHandles[p->libNum]);
        if (result != EAS_SUCCESS)
            return result;
    }
...
static EAS_RESULT JET_StartPlayback(EAS_DATA_HANDLE easHandle, EAS_I32 queueNum) {
    ...
    if (pSeg->streamHandle != NULL) {
        ...
        if ((result == EAS_SUCCESS) && (pSeg->muteFlags != 0))
            result = EAS_IntSetStrmParam(easHandle, pSeg->streamHandle, PARSER_DATA_MUTE_FLAGS,
                                         (EAS_I32) pSeg->muteFlags);
    }
    return result;
}
...
*/
EAS_INLINE EAS_INT
JET_CloseSegment (EAS_DATA_HANDLE
easHandle,
EAS_INT queueNum
)
{
...
easHandle->jetHandle->segQueue[queueNum].
streamHandle = NULL;
easHandle->jetHandle->segQueue[queueNum].
state = JET_STATE_CLOSED;
...
static EAS_RESULT JetParseInfoChunk(EAS_DATA_HANDLE easHandle, EAS_I32 pos, EAS_I32 chunkSize) {
    ...
    while ((result == EAS_SUCCESS) && (chunkSize > 0)) {
        ...
        /* get info field */
        result = EAS_HWGetDWord(easHandle->hwInstData, easHandle->jetHandle->jetFileHandle, &temp,
                                EAS_FALSE);
        if (result == EAS_SUCCESS)
            ...
            switch (infoType) {
                case INFO_NUM_SMF_CHUNKS:
                    ...
                    easHandle->jetHandle->numSegments = (EAS_U8)
                    temp;
                    break;
                ...
                case INFO_NUM_DLS_CHUNKS:
                    ...
                    easHandle->jetHandle->numLibraries = (EAS_U8)
                    temp;
                    break;
                ...
                default: { /* dpp: EAS_ReportEx(_EAS_SEVERITY_WARNING, "Ignoring unrecognized JET info type 0x%08x", infoType); */ }
                    break;
            }
...
/* open the media file */
result = EAS_HWOpenFile(easHandle->hwInstData, locator, &easHandle->jetHandle->jetFileHandle,
                        EAS_FILE_READ);
if (result != EAS_SUCCESS)
return
result;
...
/* check header */
result = EAS_HWGetDWord(easHandle->hwInstData, easHandle->jetHandle->jetFileHandle, &chunkType,
                        EAS_TRUE);
if (result == EAS_SUCCESS)
{
...
if (result == EAS_SUCCESS)
result = EAS_HWGetDWord(easHandle->hwInstData, easHandle->jetHandle->jetFileHandle, &dataSize,
                        EAS_FALSE);
...
result = EAS_HWGetDWord(easHandle->hwInstData, easHandle->jetHandle->jetFileHandle, &chunkSize,
                        EAS_FALSE);
if (result != EAS_SUCCESS)
break;
pos += 8;
...
easHandle->jetHandle->segmentOffsets[smfChunkNum++] =
pos;
else
...
result = DLSParser(easHandle->hwInstData, easHandle->jetHandle->jetFileHandle, pos,
                   &easHandle->jetHandle->libHandles[dlsChunkNum++]);
else
...
if (result != EAS_SUCCESS)
EAS_HWCloseFile(easHandle
->hwInstData, easHandle->jetHandle->jetFileHandle);
...
*/
EAS_RESULT
JET_GetAppData(EAS_DATA_HANDLE easHandle, EAS_I32 *pAppDataOffset, EAS_I32 *pAppDataSize) {
...
*/
EAS_PUBLIC EAS_RESULT
JET_Init (EAS_DATA_HANDLE
easHandle,
const S_JET_CONFIG *pConfig, EAS_INT
configSize)
{
S_JET_DATA *pJet;
EAS_U8 flags = 0;
...
for(
i = 0;
i < easHandle->jetHandle->
numLibraries;
i++) {
if(easHandle->jetHandle->libHandles[i] != NULL) {
EAS_HWFree(easHandle
->hwInstData, easHandle->jetHandle->libHandles[i]);
easHandle->jetHandle->libHandles[i] =
NULL;
}
}

EAS_HWFree(easHandle
->hwInstData, easHandle->jetHandle);
easHandle->
jetHandle = NULL;
return
result;
...
*/
EAS_PUBLIC EAS_RESULT
JET_Status (EAS_DATA_HANDLE
easHandle,
S_JET_STATUS *pStatus
)
{
S_JET_SEGMENT *pSeg;
...
{
pStatus->
currentUserID = pSeg->userID;
pStatus->
segmentRepeatCount = pSeg->repeatCount;
}
else
{
pStatus->
currentUserID = -1;
pStatus->
segmentRepeatCount = 0;
}

pStatus->
paused = !(easHandle->jetHandle->flags & JET_FLAGS_PLAYING);
pStatus->
numQueuedSegments = easHandle->jetHandle->numQueuedSegments;
pStatus->
currentPlayingSegment = easHandle->jetHandle->playSegment;
pStatus->
currentQueuedSegment = easHandle->jetHandle->queueSegment;
if (pSeg->streamHandle != NULL)
{
EAS_RESULT result;
EAS_I32 location;
if ((
result = EAS_GetLocation(easHandle, pSeg->streamHandle, &location)
) == EAS_SUCCESS)
if(location != 0)
{
pStatus->
...
*/
EAS_PUBLIC EAS_BOOL
JET_GetEvent (EAS_DATA_HANDLE
easHandle,
EAS_U32 *pEventRaw, S_JET_EVENT
*pEvent)
{
EAS_U32 jetEvent;
EAS_BOOL gotEvent;
...
*/
EAS_PUBLIC EAS_RESULT
JET_QueueSegment (EAS_DATA_HANDLE
easHandle,
EAS_INT segmentNum, EAS_INT
libNum,
EAS_INT repeatCount, EAS_INT
transpose,
EAS_U32 muteFlags, EAS_U8
userID)
{
...
userID = userID;
p->
repeatCount = (EAS_I16) repeatCount;
p->
transpose = (EAS_I8) transpose;
p->
libNum = (EAS_I8) libNum;
p->
muteFlags = muteFlags;
p->
state = JET_STATE_CLOSED;
...
/* open the file */
result = EAS_OpenJETStream(easHandle, easHandle->jetHandle->jetFileHandle,
                           easHandle->jetHandle->segmentOffsets[segmentNum], &p->streamHandle);
if (result != EAS_SUCCESS)
return
result;
p->
state = JET_STATE_OPEN;
...
easHandle->jetHandle->
jetFileHandle = fileHandle;
easHandle->jetHandle->
queueSegment = (EAS_U8)
JET_NextSegment(easHandle
->jetHandle->queueSegment);
return
result;
}
...
{
result = EAS_Pause(easHandle,
                   easHandle->jetHandle->segQueue[easHandle->jetHandle->playSegment].streamHandle);
if (result != EAS_SUCCESS)
return
result;
easHandle->jetHandle->segQueue[easHandle->jetHandle->playSegment].
state = JET_STATE_PAUSED;
count++;
}
}
...
*/
EAS_PUBLIC EAS_RESULT
JET_SetMuteFlags (EAS_DATA_HANDLE
easHandle,
EAS_U32 muteFlags, EAS_BOOL
sync)
{
S_JET_SEGMENT *pSeg;
...
muteFlags = muteFlags;
return
EAS_IntSetStrmParam(easHandle, pSeg
->streamHandle, PARSER_DATA_MUTE_FLAGS, (EAS_I32) muteFlags);
}
...
*/
EAS_PUBLIC EAS_RESULT
JET_SetMuteFlag (EAS_DATA_HANDLE
easHandle,
EAS_INT trackNum, EAS_BOOL
muteFlag,
EAS_BOOL sync
)
{
...
return
EAS_IntSetStrmParam(easHandle, pSeg
->streamHandle, PARSER_DATA_MUTE_FLAGS, (EAS_I32) pSeg->muteFlags);
}
...
*/
EAS_PUBLIC EAS_RESULT
JET_TriggerClip (EAS_DATA_HANDLE
easHandle,
EAS_INT clipID
)
{
...
/* process event queue */
while (
JET_ReadQueue(easHandle
->jetHandle->jetEventQueue,
...
pSeg->
state = JET_STATE_STOPPING;
easHandle->jetHandle->
playSegment = (EAS_U8)
JET_NextSegment(index);
}
}
break;
 ...
 *----------------------------------------------------------------------------
*/
void JET_Event(EAS_DATA_HANDLE easHandle, EAS_U32 segTrack, EAS_U8 channel, EAS_U8 controller,
               EAS_U8 value) {
    EAS_U32 event;
    ...
    if (controller == JET_EVENT_TRIGGER_CLIP) {
        ...
        for (i = 0; i < JET_MUTE_QUEUE_SIZE; i++) {
            ...
            if ((easHandle->jetHandle->muteQueue[i] & JET_CLIP_ID_MASK) ==
                (value & JET_CLIP_ID_MASK)) {
                ...
                EAS_IntSetStrmParam(easHandle, pSeg->streamHandle, PARSER_DATA_MUTE_FLAGS,
                                    (EAS_I32) pSeg->muteFlags);
                return;
            }
        }
    ...
    if ((controller >= easHandle->jetHandle->config.appEventRangeLow) &&
        (controller <= easHandle->jetHandle->config.appEventRangeHigh)) {

        event |= easHandle->jetHandle->segQueue[(segTrack & JET_EVENT_SEG_MASK)
                >> JET_EVENT_SEG_SHIFT].userID << JET_EVENT_SEG_SHIFT;
#ifdef DEBUG_JET
