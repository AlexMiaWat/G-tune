...
static void DLS_UpdateEnvelope(S_SYNTH_VOICE *pVoice, S_SYNTH_CHANNEL *pChannel,
                               const S_DLS_ENVELOPE *pEnvParams, EAS_I16 *pValue,
...
*/
void
DLS_MuteVoice(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, S_SYNTH_VOICE *pVoice, EAS_I32 voiceNum) {
    S_WT_VOICE *pWTVoice;
...
void
DLS_ReleaseVoice(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, S_SYNTH_VOICE *pVoice, EAS_I32 voiceNum) {
    S_WT_VOICE *pWTVoice;
    const S_DLS_ARTICULATION *pDLSArt;
...
void DLS_SustainPedal(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, S_SYNTH_VOICE *pVoice,
                      S_SYNTH_CHANNEL *pChannel, EAS_I32 voiceNum) {
    S_WT_VOICE *pWTVoice;
    const S_DLS_ARTICULATION *pDLSArt;
 ...
 *----------------------------------------------------------------------------
*/
static EAS_I32 DLS_UpdatePhaseInc(S_WT_VOICE *pWTVoice, const S_DLS_ARTICULATION *pDLSArt,
                                  S_SYNTH_CHANNEL *pChannel, EAS_I32 pitchCents) {
    EAS_I32 temp;
...
*/
static EAS_I32
DLS_UpdateGain(S_WT_VOICE *pWTVoice, const S_DLS_ARTICULATION *pDLSArt, S_SYNTH_CHANNEL *pChannel,
               EAS_I32 gain, EAS_U8 velocity) {
    EAS_I32 temp;
 ...
 *----------------------------------------------------------------------------
*/
static void DLS_UpdateFilter(S_SYNTH_VOICE *pVoice, S_WT_VOICE *pWTVoice, S_WT_INT_FRAME *pIntFrame,
                             S_SYNTH_CHANNEL *pChannel, const S_DLS_ARTICULATION *pDLSArt) {
    EAS_I32 cutoff;
    EAS_I32 temp;
...
*/
EAS_RESULT
DLS_StartVoice(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, S_SYNTH_VOICE *pVoice, EAS_I32 voiceNum,
               EAS_U16 regionIndex) {
    S_WT_VOICE *pWTVoice;
    ...
    DLS_UpdateEnvelope(pVoice, pChannel, &pDLSArt->eg1, &pWTVoice->eg1Value,
                       &pWTVoice->eg1Increment, &pWTVoice->eg1State);
    pWTVoice->eg2State = eEnvelopeStateInit;
    DLS_UpdateEnvelope(pVoice, pChannel, &pDLSArt->eg2, &pWTVoice->eg2Value,
    ...
    DLS_UpdateEnvelope(pVoice, pChannel, &pDLSArt->eg2, &pWTVoice->eg2Value,
                       &pWTVoice->eg2Increment, &pWTVoice->eg2State);
    pVoice->gain = (EAS_I16) DLS_UpdateGain(pWTVoice, pDLSArt, pChannel, pDLSRegion->wtRegion.gain,
                                            pVoice->velocity);

#if (NUM_OUTPUT_CHANNELS == 2)
    EAS_CalcPanControl((EAS_INT) pChannel->pan - 64 + (EAS_INT) pDLSArt->pan, &pWTVoice->gainLeft,
                       &pWTVoice->gainRight);
#endif
    ...
    pWTVoice->phaseAccum = (EAS_U32) pSynth->pDLS->pDLSSamples +
                           pSynth->pDLS->pDLSSampleOffsets[pDLSRegion->wtRegion.waveIndex];
    if (pDLSRegion->wtRegion.region.keyGroupAndFlags & REGION_FLAG_IS_LOOPED) {
        ...
        pWTVoice->loopStart = pWTVoice->loopEnd =
                pWTVoice->phaseAccum + pSynth->pDLS->pDLSSampleLen[pDLSRegion->wtRegion.waveIndex] -
                1;
...
*/
EAS_BOOL
DLS_UpdateVoice(S_VOICE_MGR *pVoiceMgr, S_SYNTH *pSynth, S_SYNTH_VOICE *pVoice, EAS_I32 voiceNum,
                EAS_I32 *pMixBuffer, EAS_I32 numSamples) {
    S_WT_VOICE *pWTVoice;
    S_SYNTH_CHANNEL *pChannel;
    const S_DLS_REGION *pDLSRegion;
    const S_DLS_ARTICULATION *pDLSArt;
    ...
    intFrame.frame.gainTarget = DLS_UpdateGain(pWTVoice, pDLSArt, pChannel,
                                               pDLSRegion->wtRegion.gain, pVoice->velocity);
    ...
    switch (*pState) {
        ...
        case eEnvelopeStateAttack:
            if (*pValue < SYNTH_FULL_SCALE_EG1_GAIN) {
                ...
                *pValue = (EAS_I16) (temp < SYNTH_FULL_SCALE_EG1_GAIN ? temp
...
//          *pValue = pEnvParams->sustainLevel;
        ...
        case eEnvelopeStateDecay:
            if (*pValue > pEnvParams->sustainLevel) {
                temp = *pValue - *pIncrement;
                *pValue = (EAS_I16) (temp > pEnvParams->sustainLevel ? temp
                                                                     : pEnvParams->sustainLevel);
                return;
            }
        ...
        default: { /* dpp: EAS_ReportEx(_EAS_SEVERITY_ERROR, "Envelope in invalid state %d\n", *pState); */ }
            break;
    }
}
