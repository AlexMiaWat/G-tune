...
/* local prototypes */
static EAS_RESULT
IMY_CheckFileType(S_EAS_DATA *pEASData, EAS_FILE_HANDLE fileHandle, EAS_VOID_PTR *ppHandle,
                  EAS_I32 offset);
...
static EAS_RESULT
IMY_SetData(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_I32 param, EAS_I32 value);

static EAS_RESULT
IMY_GetData(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_I32 param, EAS_I32 *pValue);

static EAS_BOOL
IMY_PlayNote(S_EAS_DATA *pEASData, S_IMELODY_DATA *pData, EAS_I8 note, EAS_INT parserMode);

static EAS_BOOL IMY_PlayRest(S_EAS_DATA *pEASData, S_IMELODY_DATA *pData);

static EAS_BOOL
IMY_GetDuration(EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_I32 *pDuration);
...
static EAS_BOOL
IMY_GetVolume(EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_BOOL inHeader);

static EAS_BOOL IMY_GetNumber(EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_INT *temp,
                              EAS_BOOL inHeader);

static EAS_RESULT IMY_ParseHeader(S_EAS_DATA *pEASData, S_IMELODY_DATA *pData);

static EAS_I8
IMY_GetNextChar(EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_BOOL inHeader);

static EAS_RESULT
IMY_ReadLine(EAS_HW_DATA_HANDLE hwInstData, EAS_FILE_HANDLE fileHandle, EAS_I8 *buffer,
             EAS_I32 *pStartLine);
...
#endif
                } else { /* dpp: EAS_ReportEx(_EAS_SEVERITY_WARNING, "Ignoring nested repeat section\n"); */ }
                break;
...
//3 We need a different mode for metadata parsing where we don't loop at all
                    if ((parserMode == eParserModePlay) || (pData->repeatCount != 0)) {
                        ...
                        if ((result = EAS_HWFileSeek(pEASData->hwInstData, pData->fileHandle,
                                                     pData->repeatOffset)) != EAS_SUCCESS)
                            return result;
                        IMY_ReadLine(pEASData->hwInstData, pData->fileHandle, pData->buffer,
                                     &pData->startLine);
                        pData->index = 0;
            ...
            default:
                ...
                if ((c >= 'a') && (c <= 'g')) {
                    if (IMY_PlayNote(pEASData, pData, c, parserMode))
                        return EAS_SUCCESS;
                    eof = EAS_TRUE;
                } else { /* dpp: EAS_ReportEx(_EAS_SEVERITY_WARNING, "Ignoring unexpected character '%c' [0x%02x]\n", c, c); */ }
                break;
        }
    }
...
IMY_PlayNote(S_EAS_DATA *pEASData, S_IMELODY_DATA *pData, EAS_I8 note, EAS_INT parserMode) {
    ...
    switch (pData->style) {
        ...
        case 2:
            ...
            break;
        default: { /* dpp: EAS_ReportEx(_EAS_SEVERITY_WARNING, "IMY_PlayNote: Note style out of range: %d\n", pData->style); */ }
            /*lint -e{704} shift for performance */
            pData->restTicks = duration >> 4;
            break;
    }
...
IMY_GetVolume(EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_BOOL inHeader) {
    ...
    if ((temp >= 0) && (temp <= 15)) {
        if (inHeader && (temp ==
                         0)) { /* dpp: EAS_ReportEx(_EAS_SEVERITY_WARNING, "Ignoring V0 encountered in header\n"); */ }
        else
...
static EAS_RESULT IMY_ParseHeader(S_EAS_DATA *pEASData, S_IMELODY_DATA *pData) {
    ...
    for (;;) {
        ...
        if (pData->index != 0) {
            if ((result = IMY_ReadLine(pEASData->hwInstData, pData->fileHandle, pData->buffer,
                                       &pData->startLine)) != EAS_SUCCESS) {
#ifdef _DEBUG_IMELODY
        ...
        switch (token) {
            ...
            case TOKEN_END: { /* dpp: EAS_ReportEx(_EAS_SEVERITY_WARNING, "Unexpected END:IMELODY encountered\n"); */ }
                return EAS_ERROR_FILE_FORMAT;
...
IMY_GetNextChar(EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_BOOL inHeader) {
    ...
    for (;;) {
        ...
        if (!c) {
            ...
            pData->index = 0;
            pData->buffer[0] = 0;
            if (IMY_ReadLine(hwInstData, pData->fileHandle, pData->buffer, &pData->startLine) !=
                EAS_SUCCESS) {
#ifdef _DEBUG_IMELODY
